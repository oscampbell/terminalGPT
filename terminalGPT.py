...
def ask_user_for_choice(chats_dir, chat_files):
    ...
    print(blue + "+--------+----------------------------------+")
    print(blue + "|  " + magenta + " 0  " + blue + "  | " + magenta + "Start a new chat     " + blue + "            |")
    for i, file in enumerate(chat_files):
        print(blue + "+--------+----------------------------------+")
        print(blue + "|   {0:<2}   |  {1:<32}|".format(i+1, file))
    print(blue + "+--------+----------------------------------+\n")

    choice = input(white + "Enter chat index or 'exit': ")
...
def run_chat_loop(chat_file):
    while True:
        ...
        if os.path.isfile(chat_file):
            command = 'cat {} | llm --system "{}"'.format(chat_file, question)
        else:
